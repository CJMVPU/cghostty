...
const usage =
    \\Usage: cghostty +ssh [flags] [--] <ssh args...>
    \\
    \\Flags:
...
///     cghostty +ssh <flags> -- "$@"
///
/// You can also run `cghostty +ssh` directly, or alias it yourself (e.g.
/// `alias ssh='cghostty +ssh --'`) if you prefer not to use the shell
/// integration.
///
...
///      (see `cghostty +ssh-cache`) so subsequent connections skip this
...
///     host while keeping the cache in use, prefer `cghostty +ssh-cache
...
fn runInner(
    ...
    const session: struct {
        ...
        const cache: ?DiskCache = if (opts.cache) cache: {
            const path = DiskCache.defaultPath(alloc, "cghostty") catch |err| {

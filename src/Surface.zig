...
/// Owns rendering resources, worker lifecycle and shared render state.
...
pub fn handleMessage(self: *Surface, msg: Message) !void {
    switch (msg) {
        ...
        .clipboard_read => |clipboard| {
            ...
            _ = try self.clipboardHandler().startClipboardRequest(.standard, .{ .osc_52_read = clipboard });
        },

        .kitty_clipboard_read => |req| try self.clipboardHandler().kittyClipboardRead(req),

        .kitty_clipboard_write => |req| try self.clipboardHandler().kittyClipboardWrite(req),
...
fn showSurfaceFault(self: *Surface, fault: @import("SurfaceFault.zig")) void {
...

    {
        self.render.state.mutex.lockUncancelable(global.io());
        defer self.render.state.mutex.unlock(global.io());
        fault.renderFallback(self.render.state.terminal) catch |err| {
...
pub fn textCallback(self: *Surface, text: []const u8) !void {
    ...
    try self.clipboardHandler().completeClipboardPaste(text, true);
}
...
fn mouseShiftCapture(self: *const Surface, lock: bool) bool {
    ...
    if (lock) self.render.state.mutex.lockUncancelable(global.io());
    defer if (lock) self.render.state.mutex.unlock(global.io());
...
pub fn mouseButtonCallback(
    ...
    if (button == .middle and action == .press) switch (self.config.middle_click_action) {
        .ignore => {},
        .@"clipboard-paste" => {
            _ = try self.clipboardHandler().startClipboardRequest(.standard, .{ .paste = .standard });
        },
        .@"primary-paste" => {
            if (self.rt_surface.supportsClipboard(.selection)) {
                _ = try self.clipboardHandler().startClipboardRequest(.selection, .{ .paste = .selection });
            }
        },
    };
    ...
    if (button == .right and action == .press) sel: {
        ...
        switch (self.config.right_click_action) {
            ...
            .@"copy-or-paste" => if (self.io.termio.terminal.screens.active.selection) |sel| {
                ...
                self.render.state.mutex.unlock(global.io());
                defer self.render.state.mutex.lockUncancelable(global.io());
                _ = try self.clipboardHandler().startClipboardRequest(.standard, .{ .paste = .standard });
...
pub fn performBindingAction(self: *Surface, action: input.Binding.Action) !bool {
    ...
    switch (action.scoped(.surface).?) {
        ...
        .paste_from_clipboard => return (try self.clipboardHandler().startClipboardRequest(
            .standard,
            .{ .paste = .standard },
        )) == .started,

        .paste_from_selection => return (try self.clipboardHandler().startClipboardRequest(
            .selection,
            .{ .paste = .selection },
        )) == .started,
        ...
        .crash => |location| switch (location) {
            ...
            .render => {
                _ = self.render.thread.mailbox.push(global.io(), .{ .crash = {} }, .{ .forever = {} });
                self.queueRender() catch |err| {
                    // Not a big deal if this fails.
...
pub const CompleteClipboard = Clipboard.CompleteClipboard;

fn clipboardHandler(self: *Surface) Clipboard {
    const Runtime = struct {
        fn set(surface: *Surface, loc: apprt.Clipboard, contents: []const apprt.ClipboardContent, confirm: bool) !void {
            try surface.rt_surface.setClipboard(loc, contents, confirm);
        }

        fn request(surface: *Surface, loc: apprt.Clipboard, req: apprt.ClipboardRequest) !apprt.ClipboardReadResult {
            return try surface.rt_surface.clipboardRequest(loc, req);
        }
    };
    return .{
        .surface = self,
        .queue_io = queueIo,
        .scroll_bottom = scrollToBottom,
        .set_clipboard = Runtime.set,
        .request_clipboard = Runtime.request,
    };
}

/// Complete once, retaining the request only when confirmation is still needed.
pub fn completeClipboardRequest(self: *Surface, req: apprt.ClipboardRequest, complete: CompleteClipboard) !void {
    try self.clipboardHandler().completeClipboardRequest(req, complete);
}

/// Denial consumes the request and sends any protocol-required reply.
pub fn denyClipboardRequest(self: *Surface, req: apprt.ClipboardRequest) void {
    self.clipboardHandler().denyClipboardRequest(req);
}

...
pub fn init(b: *std.Build, cfg: *const Config, deps: *const SharedDeps) !GhosttyResources {
    ...
    // Shell-integration
    {
        const install_step = b.addInstallDirectory(.{
            ...
            .install_subdir = b.pathJoin(&.{ "cghostty", "shell-integration" }),
            .exclude_extensions = &.{".md"},
        });
        try steps.append(b.allocator, &install_step.step);
    }

    // The font is embedded in the core; ship its redistribution license.
    ...
    if (cfg.emit_themes) {
        if (b.lazyDependency("iterm2_themes", .{})) |upstream| {
            const install_step = b.addInstallDirectory(.{
                ...
                .install_dir = .{ .custom = "share" },
                .install_subdir = b.pathJoin(&.{ "cghostty", "themes" }),
                .exclude_extensions = &.{".md"},
            });
            try steps.append(b.allocator, &install_step.step);
        }
    ...
    // Fish shell completions
    {
        ...
        run.addArg("+fish");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(.{}), "cghostty.fish");
    ...
    // zsh shell completions
    {
        ...
        run.addArg("+zsh");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(.{}), "_cghostty");
    ...
    // bash shell completions
    {
        ...
        run.addArg("+bash");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(.{}), "cghostty.bash");

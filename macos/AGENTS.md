# macOS cghostty Application (Apple Silicon only)
...
- Use `macos/build.nu` to build the macOS app, do not use `zig build`
  ...
  - Build: `macos/build.nu [--configuration Debug] [--action build]`
  - Output: `macos/build/<configuration>/cghostty.app` (e.g. `macos/build/Debug/cghostty.app`)
...
- Test AppleScript support:
  ...
  (2) Launch and activate the app via osascript using the absolute path
      ...
      `osascript -e 'tell application "<absolute path to build/Debug/cghostty.app>" to activate'`
  ...
  (5) When done, quit via:
      `osascript -e 'tell application "<absolute path to build/Debug/cghostty.app>" to quit'`

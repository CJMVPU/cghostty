...
const SwapChain = struct {
    ...
    const buf_count = Metal.swap_chain_count;
...
const FrameState = struct {
    ...
    pub fn init(api: Metal) !FrameState {
        var commands = try Metal.Commands.init(api.device);
...
pub fn init(alloc: Allocator, options: renderer.Options) !Self {
    ...
    // GPU resources.
    var api = try Metal.init(alloc, options);
    errdefer api.deinit();
...
/// render thread. Releases GPU resources before Metal is destroyed.
...
/// atlas texture copies, cell buffers, command storage),
...
fn drawFrameLocked(
...

    {
        ...
        // Draw the debug overlay above terminal content.

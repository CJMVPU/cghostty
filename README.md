...
默认启用 `cursor-effect = smooth`，通过原生 MSL 绘制四角随移动方向变化的光标。
横向由左右边领跑和跟随，竖向由上下边领跑和跟随；斜向由最前方的角领跑，两侧角过渡，对角跟随。前沿沿垂直于移动的方向轻微展开（轴向移动最多约 15%），到位后恢复。
按本次输入步长计时，前端响应时间为 24–180ms，后端多用 20/40/60ms 收拢。各角使用减速曲线，连续输入从当前显示位置接续，并始终保留前后响应差，不会反复等待起步或逐渐丢失拖尾。停止后结束动画刷新。系统开启“减少动态效果”时停用动画。
...
设为 `cursor-effect = none` 可关闭效果。支持不透明的方块、细线和下划线光标。Vim insert 模式的细线按字符格宽度计时，原有细边不参与展开。尺寸变化时重置动画；失焦、空心或锁形光标使用正常光标。Vim 重绘或闪烁暂时隐藏光标时不绘制动画，但保留运动状态，使连续搜索跳转可以接续。

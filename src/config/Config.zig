...
/// be silently ignored.
///
/// Any additional colors specified via background, foreground, etc.
...
// Theme-owned ANSI colors. Kept in the config for rendering, cloning and
// native read access; this is not an editable user configuration option.
palette: Palette = .{},
...
/// look. Colors that have been explicitly set by the theme are never
/// overwritten.
///
...
///
/// This defaults to `false`, following standard macOS application behavior.
@"quit-after-last-window-closed": bool = false,

/// Whether to create an initial terminal window when cghostty starts.
/// If false, the application starts without a terminal window.
...
/// use the bright red color. The terminal palette is provided by the selected theme
...
pub fn allowConfigField(self: *const Config, name: []const u8) bool {
    return isUserConfigKey(name) or (self._loading_theme and std.mem.eql(u8, name, "palette"));
}
...
fn loadTheme(self: *Config, theme: Theme) !void {
    ...
    const reader = &file_reader.interface;
    var iter: cli.args.LineIterator = .{ .r = reader, .filepath = path };
    new_config._loading_theme = true;
    try new_config.loadIter(alloc_gpa, &iter);
    new_config._loading_theme = false;
    const palette_snapshot = try new_config.arenaAlloc().create(Replay.ThemePalette);
    palette_snapshot.* = .{ .value = new_config.palette, .theme = new_config._conditional_state.theme };
    try new_config._replay_steps.append(new_config.arenaAlloc(), .{ .theme_palette = palette_snapshot });

    // Setup our replay to be conditional.
    conditional: for (new_config._replay_steps.items) |*item| {
        switch (item.*) {
            .expand, .diagnostic, .theme_palette => {},
...
pub fn parseManuallyHook(
    ...
    // Palette data is replayed as a trusted theme snapshot, never as a user
    // argument that could regain permission to override the theme.
    if (self._loading_theme and (std.mem.eql(u8, arg, "--palette") or
        std.mem.startsWith(u8, arg, "--palette="))) return true;
...
fn probableCliEnvironment() bool {
    if (internal_os.launchedFromDesktop()) return false;
...
const Replay = struct {
    const ThemePalette = struct {
        value: Palette,
        theme: @FieldType(conditional.State, "theme"),
    };

    const Step = union(enum) {
        // Store separately so the color table doesn't enlarge every replay step.
        theme_palette: *const ThemePalette,
        ...
        fn clone(
            ...
            return switch (self) {
                .@"-e" => self,
                .theme_palette => |v| snapshot: {
                    const copy = try alloc.create(ThemePalette);
                    copy.* = v.*;
                    break :snapshot .{ .theme_palette = copy };
                },
        ...
        pub fn next(self: *Self) ?[]const u8 {
            while (true) {
                ...
                switch (self.slice[self.idx]) {
                    .theme_palette => |palette| snapshot: {
                        const alloc = self.config.arenaAlloc();
                        const step = self.slice[self.idx].clone(alloc) catch |err| {
                            log.warn("error cloning theme palette err={}", .{err});
                            break :snapshot;
                        };
                        if (palette.theme == self.config._conditional_state.theme)
                            self.config.palette = palette.value;
                        self.config._replay_steps.append(alloc, step) catch |err| {
                            log.warn("error replaying theme palette err={}", .{err});
                        };
                    },
...
pub const Keybinds = struct {
    ...
    pub fn init(self: *Keybinds, alloc: Allocator) !void {
...

        {
            ...
            // Standard macOS copy/paste shortcuts use Command.
...
test "theme loading correct light/dark" {
    ...
    // Light
    {
        ...
        try testing.expectEqual(@as(u8, 0x11), cfg.palette.value[1].r);
    }
    ...
    // Dark
    {
        ...
        try testing.expectEqual(@as(u8, 0x22), cfg.palette.value[1].r);
    }
    ...
    // Light to Dark
    {
        ...
        try testing.expectEqual(@as(u8, 0x22), new.palette.value[1].r);
        try testing.expectEqual(@as(usize, 0), new._diagnostics.items().len);
        var back = (try new.changeConditionalState(.{ .theme = .light })).?;
        defer back.deinit();
        try testing.expectEqual(@as(u8, 0x11), back.palette.value[1].r);
        try testing.expectEqual(@as(usize, 0), back._diagnostics.items().len);
    }
}
...
test "palette rejects user configuration and CLI overrides" {
    const t = std.testing;
    var cfg = try Config.default(t.allocator);
    defer cfg.deinit();
    const original = cfg.palette;
    try cfg.loadData(t.allocator, "palette = 1=#123456\npalette =\nfont-size = 19\n", "/tmp/config.ghostty");
    try t.expectEqual(@as(usize, 2), cfg._diagnostics.items().len);
    try t.expectEqualStrings("unknown field", cfg._diagnostics.items()[0].message);
    try t.expectEqual(@as(f32, 19), cfg.@"font-size");
    var it: TestIterator = .{ .data = &.{ "--palette=2=#abcdef", "--palette=" } };
    try cfg.loadIter(t.allocator, &it);
    try t.expectEqual(@as(usize, 4), cfg._diagnostics.items().len);
    try t.expect(original.equal(cfg.palette));
}

test "palette theme colors survive replay and user overrides remain rejected" {
    const t = std.testing;
    var td = try internal_os.TempDir.init();
    defer td.deinit();
    {
        var file = try td.dir.createFile(t.io, "theme", .{});
        defer file.close(t.io);
        var buf: [1024]u8 = undefined;
        var writer = file.writer(t.io, &buf);
        try writer.interface.writeAll("palette = 1=#123456\npalette = 2=#abcdef\n");
        try writer.end();
    }
    var path_buf: [std.fs.max_path_bytes]u8 = undefined;
    const path = path_buf[0..try td.dir.realPathFile(t.io, "theme", &path_buf)];
    var cfg = try Config.default(t.allocator);
    defer cfg.deinit();
    const arg = try std.fmt.allocPrint(t.allocator, "--theme={s}", .{path});
    defer t.allocator.free(arg);
    var it: TestIterator = .{ .data = &.{arg} };
    try cfg.loadIter(t.allocator, &it);
    try cfg.finalize();
    try t.expectEqual(@as(usize, 0), cfg._diagnostics.items().len);
    try t.expectEqual(@as(u8, 0x12), cfg.palette.value[1].r);
    try t.expectEqual(@as(u8, 0xab), cfg.palette.value[2].r);
    try cfg.loadData(t.allocator, "palette = 1=#ffffff\n", "/tmp/config.ghostty");
    try cfg.finalize();
    try t.expectEqual(@as(usize, 1), cfg._diagnostics.items().len);
    try t.expectEqual(@as(u8, 0x12), cfg.palette.value[1].r);

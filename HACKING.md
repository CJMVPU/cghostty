...
测试结果包和导出的截图需要单独管理：指定的 `--result-bundle` 路径必须尚不存在，Xcode 不会覆盖旧结果包。重复验证应在确认上一轮测试已结束、摘要已记录后，清理上一轮结果包及导出附件，再复用同一路径。失败排查期间可以保留相关结果；问题验证完毕后，只保留必要的文字摘要或明确要求归档的证据。不要通过不断添加 `v2`、`v3`、`final2` 后缀规避已有结果包。

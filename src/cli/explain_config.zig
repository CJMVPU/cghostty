...
pub const Options = struct {
    ...
    ///
    ///   cghostty +explain-config --option=font-size
    option: ?[]const u8 = null,

    /// The keybind action to explain. For example:
    ///
    ///   cghostty +explain-config --keybind=copy_to_clipboard
...
/// Examples:
///
///   cghostty +explain-config font-size
///   cghostty +explain-config copy_to_clipboard
///   cghostty +explain-config --option=font-size
///   cghostty +explain-config --keybind=copy_to_clipboard
///
/// Flags:
...
pub fn run(alloc: Allocator) !u8 {
    ...
    const name = keybind_name orelse option_name orelse positional orelse {
        ...
        try stderr_writer.interface.writeAll("Usage: cghostty +explain-config <option>\n");
        try stderr_writer.interface.writeAll("       cghostty +explain-config --option=<option>\n");
        try stderr_writer.interface.writeAll("       cghostty +explain-config --keybind=<action>\n");

...
pub fn decodeLimited(alloc: std.mem.Allocator, data: []const u8, max_bytes: usize) Error!ImageData {
    return common.decode("jpeg", alloc, data, max_bytes);
}
...
test "jpeg respects caller pixel budget" {
    try std.testing.expectError(error.Overflow, decodeLimited(std.testing.allocator, @embedFile("1x1#000000.jpg"), 3));
}

fn decodeFixture(alloc: std.mem.Allocator) !void {
    const image = try decodeLimited(alloc, @embedFile("1x1#000000.jpg"), 4);
    defer alloc.free(image.data);
    try std.testing.expectEqualSlices(u8, &.{ 0, 0, 0, 255 }, image.data);
}

test "jpeg frees intermediate allocations on failure" {
    try std.testing.checkAllAllocationFailures(std.testing.allocator, decodeFixture, .{});
}

test "jpeg rejects truncated input" {
    const data = @embedFile("1x1#000000.jpg");

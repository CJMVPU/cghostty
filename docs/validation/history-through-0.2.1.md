> 临时产物保留说明（2026-09-23）：已按要求清理历史 `/tmp/cghostty-*`、`/private/tmp/cghostty-*` 构建副本、测试结果包与导出截图。下文路径记录的是验证当时的位置，不代表这些文件仍可打开；文字验证记录保留。后续验证产物按 `HACKING.md` 的复用与清理规则管理。

## 0.2.1 内置文楷与合成样式修复（2026-09-23，未发布）

- 默认字体最终采用 **LXGW WenKai Mono 1.522 Medium**，使用此前已锁定的官方归档与内容哈希；删除 Sarasa 源码资源和构建引用。Medium 与用户 Downloads 中的文件逐字节一致，SHA-256 为 `7a674f448b15a1b3df781c3498973d77f71d270788f7f921080c1344e9d739e1`。最终应用包含完整 Medium TTF，不再包含原 Regular TTF；分发许可与归档中的 OFL 原文一致。
- 默认 **16 pt、133 列 × 33 行**。已备份并更新此机器个人配置的字体字段、`font-style = Medium` 与相关模板说明，其余有效值完全保留，包括 **157 × 43**、Molokai 和窗口位置；新默认尺寸不会覆盖个人设置。字体选择和字体列表均识别 Medium，不将它标记为 Regular。
- 修复 CoreText 字体复制丢失合成粗体状态：`Collection.add` 的初始尺寸归一化、之后的字号调整、叠加斜体与字体轴更新均保留该状态。缩放时重新计算加粗宽度。斜体矩阵原本即可被 CoreText 保留；此前缺少 Bold/Italic 文件不是样式消失的充分原因。
- 先复现像素回归测试失败，再验证修复后中英文四种样式在 16/24/12 pt 下的像素两两不同，禁用合成后四种样式一致。此前 0.2.0 的字体家族/覆盖范围检查不足以证明粗体实际生效，以下旧记录不能作为该效果的验证依据。
- 字体与配置核心测试 **382 通过、1 跳过**（`/tmp/cghostty-wenkai-core.log`）；FreeType 后端内置字体测试 **74 通过**（`/tmp/cghostty-wenkai-freetype.log`）。
- ReleaseLocal 桌面配置测试 **5 通过**，实际 `stty size` 为 `33 133`；结果 `/tmp/cghostty-021-ui.xcresult`。171 项模板示例、四种样式的字体选择、个人有效配置保留与主题解析检查通过（`/tmp/cghostty-wenkai-config-audit.log`）。
- 上述全组检查完成后，Medium 变更专项复验：字体集合、列表与配置测试 **257 通过**，FreeType 内置字体测试 **74 通过**。像素测试同时覆盖默认选择、显式指定 Medium，以及禁用合成。Medium 桌面字体/网格用例 **1 通过、0 运行时警告**，截图确认普通、合成粗体、斜体与粗斜体；结果 `/tmp/cghostty-medium-ui.xcresult`。171 项模板示例、Medium 列表筛选、配置保留及最终应用字体字节核验通过，见 `/tmp/cghostty-medium-config-audit.log`。
- 应用版本 **0.2.1 / build 11**，同步清单、三个 Xcode 应用配置及发布说明。ReleaseLocal 构建、SwiftLint、范围/资源/签名与版本检查通过。未替换 `/Applications` 中的应用，未打包、提交、打标签或发布。

## 0.2.0 配置与字体收拢（2026-09-23，未发布；字体方案已由 0.2.1 替代）

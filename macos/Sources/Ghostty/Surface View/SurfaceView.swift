...
extension Ghostty {
    struct SurfaceWrapper: View {
        ...
        var body: some View {
            ZStack {
                ...
                if let fault = surfaceView.state.fault {
                    Rectangle().fill(ghostty.config.backgroundColor)
                    SurfaceFaultView(fault: fault) {
                        surfaceView.windowRegistry.owner(of: surfaceView)?
                            .closeSurface(surfaceView, withConfirmation: true)
                    }
                } else if !surfaceView.state.healthy {
                    Rectangle().fill(ghostty.config.backgroundColor)
    ...
    struct SurfaceRepresentable: NSViewRepresentable {
        ...
        static func dismantleNSView(_ scrollView: SurfaceScrollView, coordinator: ()) {
            scrollView.dismantle()
        }
    ...
    static func moveFocus(to: SurfaceView, from: SurfaceView? = nil) {
        to.windowRegistry.owner(of: to)?.requestFocus(to: to, from: from)
    }
}

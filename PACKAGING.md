...
仅生成 macOS Apple Silicon 应用和 ZIP。项目版本为 `build.zig.zon` 的 `.version`；发布标签使用 `v<语义版本>`。本地发布构建示例：
...
打包脚本验证应用版本／构建号与仓库记录一致、Bundle ID、主程序 arm64 架构和完整签名，然后输出 `artifacts/cghostty-<版本>-macos-arm64.zip` 及 `.sha256`。`CFBundleShortVersionString` 保留 macOS 要求的三段数字，完整语义版本保存在 `CGhosttyVersion`，并用于“关于”、`+version` 和 ZIP 名称，例如 `cghostty-<版本>-dev-macos-arm64.zip`。

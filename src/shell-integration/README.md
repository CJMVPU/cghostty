...
if [ -n "${CGHOSTTY_RESOURCES_DIR}" ]; then
    builtin source "${CGHOSTTY_RESOURCES_DIR}/shell-integration/bash/ghostty.bash"
fi
```
...
For [Elvish](https://elv.sh), `$CGHOSTTY_RESOURCES_DIR/src/shell-integration`
...
with `$CGHOSTTY_RESOURCES_DIR/src/shell-integration`. It contains
...
```nushell
source $CGHOSTTY_RESOURCES_DIR/shell-integration/nushell/vendor/autoload/ghostty.nu
use ghostty *
```
...
```zsh
if [[ -n $CGHOSTTY_RESOURCES_DIR ]]; then
  source "$CGHOSTTY_RESOURCES_DIR"/shell-integration/zsh/ghostty-integration
fi
```
